...
def ASS(start_state, goal_state, heuristic):
    ...
    frontier.push(node, node.cost + heuristic(node.state, goal_state))
    numnodes = 0
    ...
    while not frontier.is_empty():
        ...
        for next_state, next_action, next_cost in node.state.successors():
            ...
            if next_state not in expanded:
                frontier.push(Node(next_state, node, next_action, node.cost + next_cost), (node.cost + next_cost) + heuristic(next_state, goal_state))
